...
/// A backend that is simply not reachable.
///
/// Every request fails the way a lost connection fails, which is how the
/// transport tells "offline" apart from "refused" — and it is the difference
/// the outbox turns on: an offline tap is queued, a refused one is not.
class OfflineBackend extends FakeBackend {
  /// Creates an unreachable backend.
  OfflineBackend() : super(<String, Object? Function(SentRequest)>{});

  @override
  http.Client get client => MockClient(
    (http.Request request) async =>
        throw http.ClientException('no connection', request.url),
  );
}

/// A dependency graph whose every request fails for lack of a connection.
Future<Dependencies> offlineDependencies() async {
  final Dependencies dependencies = Dependencies(
    environment: PartnerEnvironment(apiBaseUrl: Uri.parse('http://api.test')),
    httpClient: OfflineBackend().client,
